  ...
  >
    ...
    <div class="flex flex-wrap">
      <span
        ...
        :class="{ bg: bgColor }"
        @click="paginationClickHandling(page, index)"
      >
        {{ page }}
      </span>
    </div>
    ...
    <button
      @click="paginateNextButton"
      :disabled="useAPIDataStore.currentPage === useAPIDataStore.totalPages"
    >
      Next
    </button>
  </nav>
...
const paginationClickHandling = function (num, index) {
  if (num === index + 1) bgColor.value = true;
...
const paginatePreviousButton = function () {
  useAPIDataStore.prevPage();
  paginationClickHandling(useAPIDataStore.currentPage);
};

const paginateNextButton = function () {
  useAPIDataStore.nextPage();
  paginationClickHandling(useAPIDataStore.currentPage);
};
</script>
