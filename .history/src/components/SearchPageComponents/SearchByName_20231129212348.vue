<template>
  ...
  <nav class="w-86 m-auto justify-center m-auto flex gap-3">
    <button
...
const searchResults = function () {
  ...
  if (query) {
    useAPIDataStore.hotelSearchData.hotels =
      useAPIDataStore.realData.hotels.filter((item) =>
        item.property.name.toLowerCase().includes(query)
      );
  } else {
    useAPIDataStore.hotelSearchData = useAPIDataStore.realData;
  }
};
