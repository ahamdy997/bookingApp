<template>
  ...
  <nav class="w-full justify-center m-auto flex gap-3">
    ...
    <span
      ...
      @click="paginationClickHandling(1, $event)"
      >1</span
    >
    <span
      class="px-2 cursor-pointer rounded-xl"
      @click="paginationClickHandling(2, $event)"
      >2</span
    >
    <span
      class="px-2 cursor-pointer rounded-xl"
      @click="paginationClickHandling(3, $event)"
      >3</span
    >
    <span
      class="px-2 cursor-pointer rounded-xl"
      @click="paginationClickHandling(4, $event)"
      >4</span
    >
    <span
      class="px-2 cursor-pointer rounded-xl"
      @click="paginationClickHandling(5, $event)"
      >5</span
    >
    <button
...
const paginationClickHandling = function (num, event) {
  useAPIDataStore.currentPage = num;
  console.log(event);
  if (+event.textContent === num)
    event.target.classList.add("bg-[var(--mainBlue)]");
  else event.target.classList.remove("bg-[var(--mainBlue)]");
};
</script>
