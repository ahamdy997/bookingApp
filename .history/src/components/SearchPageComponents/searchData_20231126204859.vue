      ...
      >
        <div class="flex h-full">
              ...
              <div class="rating flex items-center">
                  ...
                  <div v-for="n in 5" :key="n">
                    <span
                      v-if="
                        n <=
                        Number(
                          data.hotels.property.reviewScore % 2 == 0
                            ? Math.round(+data.hotels.property.reviewScore * 10) /
                                2 /
                                10
                            : Math.round(
                                (+data.hotels.property.reviewScore * 10 + 1) / 10
                              ) / 2
                        )
                      "
                      class="filled"
                    ...
                    <span
                      v-else-if="
                        (n - 0.5 === Number(data.hotels.property.reviewScore % 2)) == 0
                          ? Math.round(+data.hotels.property.reviewScore * 10) / 2 / 10
                          : Math.round(
                              (+data.hotels.property.reviewScore * 10 + 1) / 10
                            ) / 2
                      "
                ...
                <p class="text-sm text-[#4f4f4f] ml-[12px]">
                  {{
                    +data.hotels.property.reviewScore % 2 == 0
                      ? Math.round(+data.hotels.property.reviewScore * 10) / 2 / 10
                      : Math.round((+data.hotels.property.reviewScore * 10 + 1) / 10) /
                        2
                  }}
                  ({{ data.hotels.property.reviewCount }}
                  Reviews)
                </p>
              </div>
          ...
          <div class="ms-auto text-end">
            <p
              v-if="
                data.hotels.property.priceBreakdown?.excludedPrice?.value != undefined
              "
            ...
            >
              ...
              {{
                Math.round(
                  ((Number(data.hotels.property.priceBreakdown.excludedPrice.value) *
                    100) /
                    Number(data.hotels.property.priceBreakdown.grossPrice.value)) *
                    100
                ) / 100
              }}% off
            </p>
            ...
            <p
              ...
              v-if="data.hotels.property.priceBreakdown?.excludedPrice?.value"
            >
              {{
                Math.round(
                  (Number(data.hotels.property.priceBreakdown.excludedPrice.value) *
            ...
            >
              <p class="text-sm text-[#EB5757] line-through">
                $ {{ data.hotels.property.priceBreakdown.grossPrice.value.toFixed(2) }}
              </p>
              <p
                class="text-xl"
                v-if="data.hotels.property.priceBreakdown?.excludedPrice?.value"
              >
                $
                {{
                  data.hotels.property.priceBreakdown.excludedPrice.value.toFixed(2)
                }}
              </p>
            </div>
...
<script setup>
import { useAPIData.hotels } from "@/stores/APIDataStore.js";
