...
const searchResults = function () {
  const realData = useAPIDataStore.hotelSearchData;
  query = hotelNameSearch.value.toLowerCase();
  if (hotelNameSearch.value !== "") {
    useAPIDataStore.hotelSearchData = useAPIDataStore.hotelSearchData.filter(
      (item) => item.property.name.toLowerCase().includes(query)
    );
  } else useAPIDataStore.hotelSearchData = realData;
};
</script>
