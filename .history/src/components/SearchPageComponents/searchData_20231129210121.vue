<template>
  <div v-if="true" class="search-result w-[915px] me-auto mb-[60px] mt-[50px]">
    <h2 class="font-semibold text-2xl w-[504px]">
      {{ useDataAlive.searchQuery }} :
      {{ parseInt(dataStore.hotelSearchData.meta.title) }} search results found
    </h2>
    <transition mode="out-in" data-vue-meta="ignore">
      <ul>
        <transition-group mode="out-in" data-vue-meta="ignore">
          <li
            v-for="data in dataStore.paginatedItems.hotels"
